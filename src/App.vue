...
export default {
  ...
  async created() {
      const _ = await fetch("https://min-api.cryptocompare.com/data/all/coinlist?summary=true");
      const data = await _.json();

      if(data.Response === "Success"){
        this.subTitles.push(data.Data)
      }
    ...
    computed: {
      ...
      filteredTickers() {
        return this.tickers.filter(ticker => ticker.name.toUpperCase()
    ...
    methods: {
      ...
      searchSubTickers(){
        ...
        for(let [key, value] of Object.entries(this.subTitles?.[0])){
          if(result.length > 3) break;
          if(value.FullName?.toUpperCase().includes(this.ticker.toUpperCase()) ||
          value.Symbol?.toUpperCase().includes(this.ticker.toUpperCase())) {
            result.push(key)
          }
        }
        return result
